...
namespace schnek {
  ...
  class Token {
    ...
    /// Copy constructor
    Token(const Token &t) : filename(t.filename), line(t.line), token(t.token), str(t.str) {}
  ...
  class TokenList {
    ...
    void reset() { tlist.clear(); }

    /// Insert an individual token onto the list.
    void insert(int line, int token) { tlist.push_back(Token(filename, line, token)); }
     ...
     */
    void insert(const TokenList &tokens) { tlist.insert(tlist.begin(), tokens.begin(), tokens.end()); }

    /// Returns an iterator to the beginning of the list
    const_iterator begin() const { return tlist.begin(); }

    /// Returns an iterator to the end of the list
    const_iterator end() const { return tlist.end(); }
  };
