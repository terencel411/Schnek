  ...
  namespace concept {
    ...
    namespace internal::grid_storage {
      ...
      struct has_method_get<
          T, IndexType, std::void_t<decltype(std::declval<T>().get(std::declval<const IndexType&>()))>>
          : std::true_type {};
      ...
      struct has_method_resize_range<
          T, std::void_t<decltype(std::declval<T>().resize(std::declval<typename T::RangeType>()))>> : std::true_type {
      };
    ...
    struct GridStorageConcept {
      ...
      static constexpr bool value = has_value_type && has_rank && has_index_type && has_range_type && has_get_method &&
                                    has_resize_method && has_resize_range_method && has_stride_method &&
